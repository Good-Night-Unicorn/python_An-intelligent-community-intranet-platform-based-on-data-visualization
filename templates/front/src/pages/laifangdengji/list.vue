...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"30px 7% 10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
      <el-form-item :style='{"alignItems":"center","margin":"0 10px","display":"flex"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>来访时间</div>
        <el-date-picker 
          v-model="timeRange"
          type="datetimerange"
          range-separator="至"
		  :style='{"border":"1px solid #eee","padding":"3px 10px","outline":"none","borderRadius":"0","background":"#fff","width":"auto","justifyContent":"center"}'
          start-placeholder="来访时间起始"
          end-placeholder="来访时间结束"
	  ...
	  <el-button v-if="isAuth('laifangdengji','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 35px","margin":"0 10px 0 0","color":"#fff","minWidth":"90px","outline":"none","borderRadius":"0","background":"#20ad9e","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/laifangdengjiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"width":"100%","padding":"20px 7%","margin":"20px 0 10px","background":"none"}'>
		...
		<div class="list1 index-pv1" :style='{"padding":"0","margin":"30px 0 0","flexWrap":"wrap","background":"none","display":"flex","width":"100%","height":"auto"}'>
			<div :style='{"width":"30%","margin":"0 3% 20px 0","position":"relative","background":"none","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				...
				<div :style='{"padding":"4px 0","margin":"0 auto 10px","whiteSpace":"nowrap","overflow":"hidden","color":"#333","textAlign":"center","background":"linear-gradient(0deg, rgba(238,177,238,1) 0%, rgba(165,228,222,1) 30%, rgba(165,228,222,1) 100%)","width":"90%","lineHeight":"30px","fontSize":"14px","textOverflow":"ellipsis"}' class="name ">{{item.laifangshijian}}</div>
			</div>
		</div>
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        if (this.timeRange.length > 0) {
          searchWhere.laifangshijianstart = this.timeRange[0];
          searchWhere.laifangshijianend = this.timeRange[1];
        }
        if (this.formSearch.zhuhuxingming != '') searchWhere.zhuhuxingming = '%' + this.formSearch.zhuhuxingming + '%';
        this.$http.get('laifangdengji/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/laifangdengjiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
