...
<div :style='{"width":"100%","padding":"40px 7%","margin":"0 auto","position":"relative","background":"none"}'>
    <el-form
      ...
      label-width="150px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0","alignItems":"center","background":"linear-gradient(90deg, rgba(238,177,238,1) 0%, rgba(165,228,222,1) 30%, rgba(165,228,222,1) 100%)","display":"flex"}'  label="高危状态" prop="gaoweizhuangtai">
            <el-select v-model="ruleForm.gaoweizhuangtai" placeholder="请选择高危状态"  >
              <el-option
                  v-for="(item,index) in gaoweizhuangtaiOptions"
                  :key="index"
                  :label="item"
                  :value="item">
              </el-option>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px 0","alignItems":"center","background":"linear-gradient(90deg, rgba(238,177,238,1) 0%, rgba(165,228,222,1) 30%, rgba(165,228,222,1) 100%)","display":"flex"}' label="温馨提示" prop="wenxintishi">
            <el-input v-model="ruleForm.wenxintishi" 
                placeholder="温馨提示" clearable ></el-input>
          </el-form-item>
...
<script>
  export default {
    data() {
      return {
        ...
        ruleForm: {
          gaoweizhuangtai: '',
          loudonghao: '',
          wenxintishi: '八号楼栋犯罪人员数目最多，请业主注意小心',
        },
        gaoweizhuangtaiOptions: [],
        rules: {
          gaoweizhuangtai: [
          ],
          loudonghao: [
    ...
    methods: {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            if(o=='gaoweizhuangtai'){
              this.ruleForm.gaoweizhuangtai = obj[o];
              this.ro.gaoweizhuangtai = true;
              continue;
            }
            if(o=='loudonghao'){
            ...
            if(o=='wenxintishi'){
              this.ruleForm.wenxintishi = obj[o];
              this.ro.wenxintishi = true;
              continue;
            }
          }
        }
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('gaoweiloudong/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
        ...
        this.$refs["ruleForm"].validate(valid => {
          if(valid) {
            ...
            if(crossrefid && crossuserid) {
                 ...
                 this.$http.get('gaoweiloudong/list', {
                  params: params
                 }).then(res => {
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('gaoweiloudong/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
