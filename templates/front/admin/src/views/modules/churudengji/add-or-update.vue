<template>
	<div class="addEdit-block" :style='{"padding":"0px"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"49%","float":"left","margin":"0 0 20px 0"}' class="input" v-if="type!='info'" label="登记编号" prop="dengjibianhao">
					<el-input v-model="ruleForm.dengjibianhao" placeholder="登记编号" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"49%","float":"left","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.dengjibianhao" label="登记编号" prop="dengjibianhao">
					<el-input v-model="ruleForm.dengjibianhao" placeholder="登记编号" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"49%","float":"left","margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="类型" prop="leixing">
				...
				<el-form-item :style='{"width":"49%","float":"left","margin":"0 0 20px 0"}' v-else class="input" label="类型" prop="leixing">
					<el-input v-model="ruleForm.leixing"
						placeholder="类型" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"49%","float":"left","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="登记时间" prop="dengjishijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"49%","float":"left","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.dengjishijian" label="登记时间" prop="dengjishijian">
					<el-input v-model="ruleForm.dengjishijian" placeholder="登记时间" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"49%","float":"left","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="备注" prop="beizhu">
					<el-input v-model="ruleForm.beizhu" placeholder="备注" clearable  :readonly="ro.beizhu"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"49%","float":"left","margin":"0 0 20px 0"}' v-else class="input" label="备注" prop="beizhu">
		...
		init(id,type) {
			...
			this.$http({
				...
				if (data && data.code === 0) {
					...
					var json = data.data;
					if(((json.zhuhuzhanghao!=''&&json.zhuhuzhanghao) || json.zhuhuzhanghao==0) && this.$storage.get("role")!="管理员"){
						this.ruleForm.zhuhuzhanghao = json.zhuhuzhanghao
						this.ro.zhuhuzhanghao = true;
					}
					if(((json.zhuhuxingming!=''&&json.zhuhuxingming) || json.zhuhuxingming==0) && this.$storage.get("role")!="管理员"){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `churudengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.churudengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
