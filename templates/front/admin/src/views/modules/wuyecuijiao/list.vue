		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"margin":"0 0 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"20px 30px","borderColor":"#565656 #4d4d4d #aaa","borderStyle":"solid","background":"","borderWidth":"0px 0px 0px 0","display":"block"}' >
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}' class="select" label="月份" prop="yuefen">
						<label :style='{"margin":"0 12px 0 0","color":"#aaa","display":"inline-block","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">月份</label>
						<el-select  @change="yuefenChange" clearable v-model="searchForm.yuefen" placeholder="请选择月份">
					...
					<div :style='{"margin":"0 10px 0 0","display":"inline-block"}' class="select" label="缴费类型" prop="jiaofeileixing">
						<label :style='{"margin":"0 12px 0 0","color":"#aaa","display":"inline-block","lineHeight":"40px","fontSize":"16px","fontWeight":"500","height":"40px"}' class="item-label">缴费类型</label>
						<el-select  @change="jiaofeileixingChange" clearable v-model="searchForm.jiaofeileixing" placeholder="请选择缴费类型">
				...
				<el-row :style='{"padding":"0 30px","margin":"20px 0","display":"flex"}'>
					<el-button :style='{"border":"1px solid #113355","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"2px","background":"-webkit-linear-gradient(top,#446688,#113355)","width":"auto","fontSize":"14px","height":"30px"}' v-if="isAuth('wuyecuijiao','新增')" type="success" @click="addOrUpdateHandler()">新增</el-button>
					<el-button :style='{"border":"1px solid #b40b0c","cursor":"pointer","padding":"0 24px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"2px","background":"-webkit-linear-gradient(top,#e23738,#ca1112)","width":"auto","fontSize":"14px","height":"30px"}' v-if="isAuth('wuyecuijiao','删除')" :disabled="dataListSelections.length <= 0" type="danger" @click="deleteHandler()">删除</el-button>
			...
			<!-- <div> -->
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='false' prop="feiyongqingdan" width="200" label="费用清单">
						<template slot-scope="scope">
							<div v-if="scope.row.feiyongqingdan">
								<img v-if="scope.row.feiyongqingdan.substring(0,4)=='http'" :src="scope.row.feiyongqingdan.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.feiyongqingdan.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column :resizable='true' :sortable='false' prop="ispay" label="是否支付">
						<template slot-scope="scope">
							...
							<el-button v-if="scope.row.ispay!='已支付' && isAuth('wuyecuijiao','支付') " type="text" size="small" @click="payHandler(scope.row)">支付</el-button>
						</template>
					</el-table-column>
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button :style='{"border":"1px solid #3ca512","cursor":"pointer","padding":"0 14px","margin":"3px 6px 3px 0","outline":"none","color":"#fff","borderRadius":"4px","background":"-webkit-linear-gradient(top,#8ad96b,#41b314)","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('wuyecuijiao','查看')" type="success" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button :style='{"border":"1px solid #409eff","cursor":"pointer","padding":"0 14px","margin":"3px 6px 3px 0","outline":"none","color":"#fff","borderRadius":"4px","background":"-webkit-linear-gradient(top,#81b5ea,#2e89e6)","width":"auto","fontSize":"14px","height":"32px"}' v-if=" isAuth('wuyecuijiao','修改')" type="primary" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>





							<el-button :style='{"border":"1px solid #c01515","cursor":"pointer","padding":"0 14px","margin":"3px 6px 3px 0","outline":"none","color":"#fff","borderRadius":"4px","background":"-webkit-linear-gradient(top,#de6464,#db2222)","width":"auto","fontSize":"14px","height":"32px"}' v-if="isAuth('wuyecuijiao','删除') " type="danger" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
				<el-pagination
...
export default {
  ...
  methods: {
    ...
    payHandler(row){
      this.$storage.set('paytable','wuyecuijiao');
